...
class QuanbyGeotaggingSupabaseUser extends BaseAuthUser {
  QuanbyGeotaggingSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> quanbyGeotaggingSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = QuanbyGeotaggingSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
